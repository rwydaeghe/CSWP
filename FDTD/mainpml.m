...
x_bron=round(nx/2+n); y_bron=round(ny/2+n);
...
global ox oy p pp 
ox = zeros(ny+2*n, nx+2*n+1); oy = zeros(ny+2*n+1, nx+2*n); p = zeros(ny+2*n, nx+2*n); pp = zeros(ny+2*n, nx+2*n);
...
for it=1:nt
   ...
   step_SIT_SIP_impedancepml(nx,ny,c,dx,dy,dt,n)   %propagatie over 1 tijdstap - propagate over one time step
   ...
   pcolor(p+pp);view(0,90);axis equal;shading interp;caxis([-0.002*A 0.002*A]);title([num2str(it) '/' num2str(nt)]);hold on;
   xlim([1 nx++2*n+1]);ylim([1 ny+2*n+1]);
